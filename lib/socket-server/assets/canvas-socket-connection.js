(function () {
	...
	// create socket
	var socket = io.connect(CANVAS_CONFIG.socketHost + ':' + CANVAS_CONFIG.socketPort + '/canvas');
	...
	socket.on('highlight', function (d) {
		...
		if ($highlight) {
			...
			// remove the hightlight class from old 
	...
	var actions = {
		...
		// add element
		addElement: function ($editable, basicData) {

			var data = _.clone(basicData);

			data.element = prompt('Input the html you wish to add');


			data.after = prompt('after element') || false;
			data.before = prompt('before element') || false;
	...
	function buildContextMenu() {
		...
		var contextMenuOptions = {
			...
			items: {
				...
				"addElement": { name: "Add html", icon: 'edit' },
				"sep1": "---------",
				"remove": { name: "Remove", icon: "quit" }
			}
		};

		// variable to hold the element hovered
		var $pressTarget;
		...
		$('body').hammer({}).on("press", function (event) {
			// event.preventDefault();
			// $(event.currentTarget).contextMenu();

			console.log('pressed')

			console.log($pressTarget);

			$pressTarget.contextMenu();
		});

		// add doubletap event
		var hammerInstance = $('body').data('hammer');
